...
class PGD():
    def __init__(self, classifier, detector, classifier_loss=None, detector_loss=None, steps=100, alpha=1/255, eps=8/255, use_projection=True, projection_norm='linf', target=None, lmbd=0, k=None, project_detector=False, project_classifier=False, img_min=0, img_max=1):
        '''
...
#         alarm_targets = torch.tensor(np.zeros(len(inputs)).reshape(-1, 1))
        ...
        # targeted attack
        if self.target:
            targeted_targets = torch.tensor(torch.tensor(self.target * np.ones(len(inputs)), dtype=torch.int64)).to(device)
